...
class Loja(GameState):
    ...
    def abrir_loja(self, carro, info, conta):
        ...
        som_menu = pygame.mixer.Sound("assets/audios/menu.mp3")
        som_cash = pygame.mixer.Sound("assets/audios/cash.mp3")
        moeda = pygame.image.load('assets/sprites/coin_1.png')
        pygame.init()
        ...
        while self.run:
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_SPACE:
                        if not info.get_comprou(posicao_cursor):
                            if conta.dinheiro >= info.get_preco(posicao_cursor):
                                conta.gastar_dinheiro(info.get_preco(posicao_cursor))
                                if posicao_cursor != 4:
                                    som_cash.play()
            ...
            self.escrever_texto_opcoes('$' + str(conta.dinheiro), fonte, 100, 600, tela)
            tela.blit(moeda, (220, 560))
...
if __name__ == '__main__':
    ...
    info = InfoCarro()
    conta = conta_jogador()
    loja.abrir_loja(skin, info, conta)
