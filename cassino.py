...
class Cassino(GameState):
    ...
    def abrir_cassino(self, info, conta):
        ...
        while self.run:
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE:
                        if conta.fichas > 0:
                            conta.diminuir_ficha()
                            roda = True
                        elif conta.dinheiro >= 20:
                            roda = True
                            conta.gastar_dinheiro(20)
                    if event.key == pygame.K_ESCAPE:
                        self.set_next_state('Menu')
                        self.set_run(False)
            tela.fill((100, 100, 100))
            tela.blit(roleta_tela, (350, 0))
            self.escrever_texto_opcoes('$' + str(conta.dinheiro), fonte, 1280, 700, tela)
            tela.blit(items['moeda'][0],(1400,660))
            self.escrever_texto_opcoes(str(conta.fichas), fonte, 1300, 600, tela)
            tela.blit(items['ficha cassino'][0], (1350,570))
            # Laranja (Casa 7)
            criar_retangulo((246, 118, 34), 1325, 425, 100, 100, tela)
            self.escrever_texto_opcoes('$40', fonte, 1330, 450, tela)

            # Amarelo (Casa 6)
            criar_retangulo((254, 174, 53), 75, 300, 100, 100, tela)
            self.escrever_texto_opcoes('$20', fonte, 80, 325, tela)
            ...
            self.escrever_texto_opcoes('$20', fonte, 80, 200, tela)
            ...
            if info.get_comprou(4):
                self.escrever_texto_opcoes('$40', fonte, 1330, 200, tela)
            else:
            ...
            if roda:
                ...
                if casa == 0:
                    ...
                    if not info.get_comprou(4):
                        ...
                        conta.depositar_dinheiro(40)
                        self.escrever_texto_opcoes('Você ganhou $40', fonte, 500, 600, tela)
                    pygame.display.flip()
                    pygame.time.wait(2000)
                    ...
                    self.escrever_texto_opcoes('Você ganhou $20', fonte, 500, 600, tela)
                    pygame.display.flip()
                    pygame.time.wait(2000)
